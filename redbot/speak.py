...
class URI_TOO_LONG(Note):
    ...
    text = u"""Long URIs aren't supported by some implementations, including
...
class URI_BAD_SYNTAX(Note):
    ...
    text = u"""This isn't a valid URI. Look for illegal characters and other
...
class REQUEST_HDR_IN_RESPONSE(Note):
    ...
    text = u"""%(field_name)s is only defined to have meaning in requests;
...
class RESPONSE_HDR_IN_REQUEST(Note):
    ...
    level = l.BAD
    summary = u'"%(field_name)s" is a request header.'
    text = u"""%(field_name)s is only defined to have meaning in responses;
...
class FIELD_NAME_BAD_SYNTAX(Note):
    ...
    text = u"""Header names are limited to the TOKEN production in HTTP; i.e., 
...
class HEADER_BLOCK_TOO_LARGE(Note):
    ...
    text = u"""Some implementations have limits on the total size of headers
...
class HEADER_TOO_LARGE(Note):
    ...
    text = u"""Some implementations limit the size of any single header
    line."""
...
class HEADER_NAME_ENCODING(Note):
    ...
    text = u"""HTTP header field-names can only contain ASCII characters. RED
...
class HEADER_VALUE_ENCODING(Note):
    ...
    text = u"""HTTP headers use the ISO-8859-1 character set, but in most
...
class HEADER_DEPRECATED(Note):
    ...
    text = u"""This header field is no longer recommended for use, because of
...
class SINGLE_HEADER_REPEAT(Note):
    ...
    text = u"""This header is designed to only occur once in a message. When it
...
class BODY_NOT_ALLOWED(Note):
    ...
    text = u"""HTTP defines a few special situations where a response does not
...
class BAD_SYNTAX(Note):
    ...
    text = u"""The value for this header doesn't conform to its specified 
...
class BAD_CC_SYNTAX(Note):
    category = c.CACHING
    level = l.BAD
    summary = u"The %(bad_cc_attr)s Cache-Control directive's syntax is \
incorrect."
...
class AGE_NOT_INT(Note):
    ...
    text = u"""The <code>Age</code> header indicates the age of the response;
...
class BAD_CHUNK(Note):
    ...
    text = u"""The response indicates it uses HTTP chunked encoding, but there
...
valid."
    text = u"""GZip-compressed responses have a header that contains metadata.
...
class BAD_ZLIB(Note):
    ...
    text = u"""GZip-compressed responses use zlib compression to reduce the 
...
class ENCODING_UNWANTED(Note):
    ...
    text = u"""%(response)s's <code>Content-Encoding</code> header indicates 
...
class TRANSFER_CODING_IDENTITY(Note):
    ...
    text = u"""HTTP defines <em>transfer-codings</em> as a hop-by-hop encoding
...
class TRANSFER_CODING_UNWANTED(Note):
    ...
    text = u"""%(response)s's <code>Transfer-Encoding</code> header indicates 
...
class TRANSFER_CODING_PARAM(Note):
    ...
    text = u"""HTTP allows transfer-codings in the
...
class BAD_DATE_SYNTAX(Note):
    ...
    text = u"""HTTP dates have very specific syntax, and sending an invalid 
...
class LM_FUTURE(Note):
    ...
    text = u"""The <code>Last-Modified</code> header indicates the last point 
...
class CONTENT_TRANSFER_ENCODING(Note):
    ...
    text = u"""<code>Content-Transfer-Encoding</code> is a MIME header, not
...
class MIME_VERSION(Note):
    ...
    text = u"""<code>MIME_Version</code> is a MIME header, not a HTTP header; 
...
directive."
    text = u"""<code>Pragma</code> is a very old request header that is 
...
class PRAGMA_OTHER(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"""The Pragma header is being used in an undefined way."""
    text = u"""HTTP only defines <code>Pragma: no-cache</code>; other uses of
...
class VIA_PRESENT(Note):
    ...
    text = u"""The <code>Via</code> header indicates that one or more
...
class LOCATION_UNDEFINED(Note):
    ...
    text = u"""The <code>Location</code> header is used for specific purposes
...
class LOCATION_NOT_ABSOLUTE(Note):
    ...
    text = u"""<code>Location</code> was originally specified to contain 
...
media type."
    text = u"""Many Web browers "sniff" the media type of responses to figure
...
class CONTENT_TYPE_OPTIONS_UNKNOWN(Note):
    category = c.SECURITY
    level = l.WARN
    summary = u"%(response)s contains an X-Content-Type-Options header with an \
unknown value."
    text = u"""Only one value is currently defined for this header,
...
class DOWNLOAD_OPTIONS(Note):
    category = c.SECURITY
    level = l.INFO
    summary = u"%(response)s can't be directly opened directly by Internet \
Explorer when downloaded."
    text = u"""When the <code>X-Download-Options</code> header is present
...
class DOWNLOAD_OPTIONS_UNKNOWN(Note):
    category = c.SECURITY
    level = l.WARN
    summary = u"%(response)s contains an X-Download-Options header with an \
unknown value."
    text = u"""Only one value is currently defined for this header,
...
class FRAME_OPTIONS_DENY(Note):
    category = c.SECURITY
    level = l.INFO
    summary = u"%(response)s prevents some browsers from rendering it if it \
will be contained within a frame."
    text = u"""The <code>X-Frame-Options</code> response header controls how
...
class FRAME_OPTIONS_UNKNOWN(Note):
    category = c.SECURITY
    level = l.WARN
    summary = u"%(response)s contains an X-Frame-Options header with an unknown \
value."
    text = u"""Only two values are currently defined for this header,
...
class SMART_TAG_NO_WORK(Note):
    ...
    text = u"""This header doesn't have any effect on Microsoft Smart Tags,
...
class UA_COMPATIBLE(Note):
    category = c.GENERAL
    level = l.INFO
    summary = u"%(response)s explicitly sets a rendering mode for Internet \
Explorer 8."
    text = u"""Internet Explorer 8 allows responses to explicitly set the
...
class UA_COMPATIBLE_REPEAT(Note):
    category = c.GENERAL
    level = l.BAD
    summary = u"%(response)s has multiple X-UA-Compatible directives targeted \
at the same UA."
    text = u"""Internet Explorer 8 allows responses to explicitly set the
...
class XSS_PROTECTION_ON(Note):
    ...
    text = u"""Recent versions of Internet Explorer have built-in Cross-Site
...
class RANGE_SUBREQ_PROBLEM(Note):
    ...
    text = u"""When RED tried to check the resource for partial content
...
class UNKNOWN_RANGE(Note):
    ...
    text = u"""The <code>Accept-Ranges</code> response header tells clients
...
class RANGE_CORRECT(Note):
    ...
    text = u"""This resource advertises support for ranged requests with
...
class RANGE_CHANGED(Note):
    ...
    text = u"""A new representation was retrieved when checking support of
...
class RANGE_STATUS(Note):
    ...
    text = u"""This resource advertises support for ranged requests; that is,
...
class RANGE_NEG_MISMATCH(Note):
    category = c.RANGE
    level = l.BAD
    summary = u"Partial responses don't have the same support for compression \
that full ones do."
    text = u"""This resource supports ranged requests and also supports
...
class MISSING_HDRS_206(Note):
    ...
    text = u"""HTTP requires <code>206 Parital Content</code> responses to 
...
class CL_CORRECT(Note):
    ...
    text = u"""<code>Content-Length</code> is used by HTTP to delimit messages;
...
class CMD5_CORRECT(Note):
    ...
    text = u"""<code>Content-MD5</code> is a hash of the body, and can be used
...
class CONNEG_SUBREQ_PROBLEM(Note):
    ...
    text = u"""When RED tried to check the resource for content negotiation
    support, there was a problem:<p>
    <code>%(problem)s</code><p>
    Trying again might fix it."""
...
class CONNEG_GZIP_GOOD(Note):
    category = c.CONNEG
    level = l.GOOD
    summary = u'Content negotiation for gzip compression is supported, saving \
%(savings)s%%.'
    text = u"""HTTP supports compression of responses by negotiating for
...
class CONNEG_GZIP_BAD(Note):
    category = c.CONNEG
    level = l.WARN
    summary = u'Content negotiation for gzip compression makes the response \
%(savings)s%% larger.'
    text = u"""HTTP supports compression of responses by negotiating for
    <code>Content-Encoding</code>. When RED asked for a compressed response,
...
class CONNEG_NO_VARY(Note):
    category = c.CONNEG
    level = l.BAD
    summary = u"%(response)s is negotiated, but doesn't have an appropriate \
Vary header."
    text = u"""All content negotiated responses need to have a
...
class VARY_INCONSISTENT(Note):
    ...
    text = u"""HTTP requires that the <code>Vary</code> response header be sent
...
class VARY_STATUS_MISMATCH(Note):
    category = c.CONNEG
    level = l.WARN
    summary = u"The response status is different when content negotiation \
happens."
    text = u"""When content negotiation is used, the response status
...
class VARY_HEADER_MISMATCH(Note):
    category = c.CONNEG
    level = l.BAD
    summary = u"The %(header)s header is different when content negotiation \
happens."
    text = u"""When content negotiation is used, the %(header)s response
...
class VARY_BODY_MISMATCH(Note):
    ...
    text = u"""When content negotiation is used, the response body typically
...
class VARY_ETAG_DOESNT_CHANGE(Note):
    ...
    text = u"""HTTP requires that the <code>ETag</code>s for two different
...
class DATE_CORRECT(Note):
    ...
    text = u"""HTTP's caching model assumes reasonable synchronisation between
...
class DATE_INCORRECT(Note):
    ...
    text = u"""Using RED's local clock, the server's clock does not appear to 
...
class AGE_PENALTY(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"It appears that the Date header has been changed by an \
intermediary."
    text = u"""It appears that this response has been cached by a reverse
...
class DATE_CLOCKLESS(Note):
    ...
    text = u"""Although HTTP allowes a server not to send a <code>Date</code>
...
class DATE_CLOCKLESS_BAD_HDR(Note):
    category = c.CACHING
    level = l.BAD
    summary = u"Responses without a Date aren't allowed to have Expires or \
Last-Modified values."
    text = u"""Because both the <code>Expires</code> and
...
capitalisation."
    text = u"""Cache-Control directive names are case-sensitive, and will not
...
class CC_DUP(Note):
    ...
    text = u"""The %(cc)s Cache-Control directive is only defined to appear
...
class NO_STORE(Note):
    ...
    text = u"""The <code>Cache-Control: no-store</code> directive indicates
...
class PRIVATE_CC(Note):
    ...
    text = u"""The <code>Cache-Control: private</code> directive indicates that
...
class PRIVATE_AUTH(Note):
    ...
    level = l.INFO
    summary = u"%(response)s only allows a private cache to store it."
    text = u"""Because the request was authenticated and this response doesn't
...
class STOREABLE(Note):
    category = c.CACHING
    level = l.INFO
    summary = u"""%(response)s allows all caches to store it."""
    text = u"""A cache can store this response; it may or may not be able to
...
class NO_CACHE(Note):
    ...
    text = u"""The <code>Cache-Control: no-cache</code> directive means that
...
class VARY_ASTERISK(Note):
    ...
    text = u"""<code>Vary *</code> indicates that responses for this resource
...
class VARY_USER_AGENT(Note):
    ...
    text = u"""Sending <code>Vary: User-Agent</code> requires caches to store
...
class VARY_COMPLEX(Note):
    ...
    text = u"""The <code>Vary</code> mechanism allows a resource to describe
...
class PUBLIC(Note):
    ...
    text = u"""The <code>Cache-Control: public</code> directive makes a
...
class FRESHNESS_FRESH(Note):
    ...
    text = u"""A response can be considered fresh when its age (here,
...
class FRESHNESS_STALE_CACHE(Note):
    ...
    text = u"""An HTTP response is stale when its age (here, %(current_age)s)
...
class FRESHNESS_STALE_ALREADY(Note):
    ...
    text = u"""A cache considers a HTTP response stale when its age (here,
...
class FRESHNESS_HEURISTIC(Note):
    category = c.CACHING
    level = l.WARN
    summary = u"%(response)s allows a cache to assign its own freshness \
lifetime."
    text = u"""When responses with certain status codes don't have explicit
...
class FRESHNESS_NONE(Note):
    category = c.CACHING
    level = l.INFO
    summary = u"%(response)s can only be served by a cache under exceptional \
circumstances."
    text = u"""%(response)s doesn't have explicit freshness information (like
...
class FRESH_SERVABLE(Note):
    ...
    text = u"""HTTP allows stale responses to be served under some
...
class FRESH_MUST_REVALIDATE(Note):
    ...
    text = u"""The <code>Cache-Control: must-revalidate</code> directive
...
class FRESH_PROXY_REVALIDATE(Note):
    category = c.CACHING
    level = l.INFO
    summary = u"%(response)s cannot be served by a shared cache once it becomes \
stale."
    text = u"""The presence of the <code>Cache-Control: proxy-revalidate</code>
...
class STALE_PROXY_REVALIDATE(Note):
    category = c.CACHING
    level = l.INFO
    summary = u"%(response)s cannot be served by a shared cache, because it is \
stale."
    text = u"""The presence of the <code>Cache-Control: proxy-revalidate</code>
    and/or <code>s-maxage</code> directives forbids shared caches (e.g., proxy
    caches) from using stale responses to satisfy requests.<p>For example,
...
class CHECK_SINGLE(Note):
    category = c.CACHING
    level = l.WARN
    summary = u"Only one of the pre-check and post-check Cache-Control \
directives is present."
    text = u"""Microsoft Internet Explorer implements two
...
class CHECK_NOT_INTEGER(Note):
    category = c.CACHING
    level = l.WARN
    summary = u"One of the pre-check/post-check Cache-Control directives has \
a non-integer value."
    text = u"""Microsoft Internet Explorer implements two
     <code>Cache-Control</code> extensions, <code>pre-check</code> and
     <code>post-check</code>, to give more control over how its cache stores
...
class CHECK_ALL_ZERO(Note):
    category = c.CACHING
    level = l.WARN
    summary = u"The pre-check and post-check Cache-Control directives are both \
'0'."
    text = u"""Microsoft Internet Explorer implements two
     <code>Cache-Control</code> extensions, <code>pre-check</code> and
     <code>post-check</code>, to give more control over how its cache stores
...
class CHECK_POST_BIGGER(Note):
    category = c.CACHING
    level = l.WARN
    summary = u"The post-check Cache-control directive's value is larger \
than pre-check's."
    text = u"""Microsoft Internet Explorer implements two
     <code>Cache-Control</code> extensions, <code>pre-check</code> and
     <code>post-check</code>, to give more control over how its cache stores
...
class CHECK_POST_PRE(Note):
    category = c.CACHING
    level = l.INFO
    summary = u"%(response)s may be refreshed in the background by Internet \
Explorer."
    text = u"""Microsoft Internet Explorer implements two
     <code>Cache-Control</code> extensions, <code>pre-check</code> and
     <code>post-check</code>, to give more control over how its cache stores
...
class NO_DATE_304(Note):
    ...
    text = u"""HTTP requires <code>304 Not Modified</code> responses to 
...
class ETAG_SUBREQ_PROBLEM(Note):
    ...
    text = u"""When RED tried to check the resource for ETag validation
    support, there was a problem:<p>
    <code>%(problem)s</code><p>
    Trying again might fix it."""
...
class INM_304(Note):
    ...
    text = u"""HTTP allows clients to make conditional requests to see if a 
...
class INM_DUP_ETAG_WEAK(Note):
    category = c.VALIDATION
    level = l.INFO
    summary = u"During validation, the ETag didn't change, even though the \
response body did."
    text = u"""<code>ETag</code>s are supposed to uniquely identify the
...
class INM_UNKNOWN(Note):
    category = c.VALIDATION
    level = l.INFO
    summary = u"An If-None-Match conditional request returned the full \
...
class INM_STATUS(Note):
    category = c.VALIDATION
    level = l.INFO
    summary = u"An If-None-Match conditional request returned a %(inm_status)s \
status."
    text = u"""HTTP allows clients to make conditional requests to see if a 
    copy that they hold is still valid. Since this response has an 
    <code>ETag</code>, clients should be able to use an 
...
class LM_SUBREQ_PROBLEM(Note):
    ...
    text = u"""When RED tried to check the resource for Last-Modified validation
    support, there was a problem:<p>
    <code>%(problem)s</code><p>
    Trying again might fix it."""
class IMS_304(Note):
...
class IMS_FULL(Note):
    category = c.VALIDATION
    level = l.WARN
    summary = u"An If-Modified-Since conditional request returned the full \
content unchanged."
    text = u"""HTTP allows clients to make conditional requests to see if a 
    copy that they hold is still valid. Since this response has a
    <code>Last-Modified</code> header, clients should be able to use an
...
class IMS_STATUS(Note):
    category = c.VALIDATION
    level = l.INFO
    summary = u"An If-Modified-Since conditional request returned a \
%(ims_status)s status."
    text = u"""HTTP allows clients to make conditional requests to see if a 
    copy that they hold is still valid. Since this response has a
    <code>Last-Modified</code> header, clients should be able to use an
...
class UNEXPECTED_CONTINUE(Note):
    ...
    text = u"""HTTP allows clients to ask a server if a request with a body
...
class UPGRADE_NOT_REQUESTED(Note):
    ...
    text = u"""HTTP defines the <code>Upgrade</code> header as a means
...
class CREATED_SAFE_METHOD(Note):
    ...
    text = u"""The <code>201 Created</code> status code indicates that
...
class CONTENT_RANGE_MEANINGLESS(Note):
    ...
    text = u"""HTTP only defines meaning for the <code>Content-Range</code>
...
class PARTIAL_WITHOUT_RANGE(Note):
    ...
    text = u"""The <code>206 Partial Response</code> status code indicates
...
class REDIRECT_WITHOUT_LOCATION(Note):
    ...
    text = u"""The %(status)s status code redirects users to another URI. 
...
class STATUS_DEPRECATED(Note):
    ...
    text = u"""When a status code is deprecated, it should not be used,
...
class STATUS_RESERVED(Note):
    ...
    text = u"""Reserved status codes can only be used by future, standard 
...
class STATUS_NONSTANDARD(Note):
    ...
    text = u"""Non-standard status codes are not well-defined and 
...
class STATUS_NOT_FOUND(Note):
    ...
    text = u"""The server couldn't find any resource to serve for the
     given URI."""
...
class STATUS_GONE(Note):
    ...
    text = u"""The server previously had a resource at the given URI, but it
     is no longer there."""
...
class STATUS_REQUEST_ENTITY_TOO_LARGE(Note):
    ...
    text = u"""The server rejected the request because the request body sent
     was too large."""
...
class STATUS_URI_TOO_LONG(Note):
    ...
    text = u"""The %(status)s status code means that the server can't or 
...
class PARAM_STAR_QUOTED(Note):
    ...
    text = u"""Parameter values that end in '*' have a specific format,
...
class PARAM_STAR_BAD(Note):
    category = c.GENERAL
    level = l.BAD
    summary = u"The %(param)s* parameter isn't allowed on the %(field_name)s \
header."
    text = u"""Parameter values that end in '*' are reserved for 
...
class PARAM_STAR_CHARSET(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"The %(param)s parameter's value uses an encoding other than \
UTF-8."
    text = u"""Parameter values that end in '*' have a specific format,
     defined in <a href="http://tools.ietf.org/html/rfc5987">RFC5987</a>,
     to allow non-ASCII text.<p>. 
...
class PARAM_REPEATS(Note):
    ...
    text = u"""Parameters on the %(field_name)s header should not repeat; 
     implementations may handle them differently."""

class PARAM_SINGLE_QUOTED(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"The '%(param)s' parameter on the %(field_name)s header is \
single-quoted."
    text = u"""The <code>%(param)s</code>'s value on the %(field_name)s 
...
class DISPOSITION_UNKNOWN(Note):
    ...
    text = u"""The <code>Content-Disposition<code> header has two 
...
class DISPOSITION_OMITS_FILENAME(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"The Content-Disposition header doesn't have a 'filename' \
parameter."
    text = u"""The <code>Content-Disposition</code> header suggests a 
...
class DISPOSITION_FILENAME_PERCENT(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"The 'filename' parameter on the Content-Disposition header \
...
class DISPOSITION_FILENAME_PATH_CHAR(Note):
    category = c.GENERAL
    level = l.WARN
    summary = u"The filename in the Content-Disposition header contains a \
path character."
    text = u"""The <code>Content-Disposition</code> header suggests a 
     filename for clients to use when saving the file locally, using 
...
class LINK_REV(Note):
    ...
    text = u"""The <code>Link</code> header, defined by 
...
class SET_COOKIE_NO_VAL(Note):
    ...
    text = u"""This <code>Set-Cookie</code> header can't be parsed into a 
...
class SET_COOKIE_NO_NAME(Note):
    ...
    text = u"""This <code>Set-Cookie</code> header has an empty name; there
...
date."
    text = u"""The <code>expires</code> date on this <code>Set-Cookie</code>
...
class SET_COOKIE_EMPTY_MAX_AGE(Note):
    ...
    text = u"""The <code>max-age</code> parameter on this
...
class SET_COOKIE_EMPTY_DOMAIN(Note):
    ...
    text = u"""The <code>domain</code> parameter on this
...
class SET_COOKIE_UNKNOWN_ATTRIBUTE(Note):
    category = c.GENERAL
    level=l.WARN
    summary = u"The %(cookie_name)s Set-Cookie header has an unknown attribute, \
'%(attribute)s'."
    text = u"""This <code>Set-Cookie</code> header has an extra parameter,
     "%(attribute)s".<p>
